...
from utils import live_weather  # noqa: E402
...
from app.schemas import (  # noqa: E402
    ...
    ModelInfo, HealthResponse, CacheStatsResponse, LiveWeatherResponse,
)
from app import weather_ml_routes  # noqa: E402
from app.weather_ml_routes import (  # noqa: E402
    router as weather_ml_router, load_weather_ml_engine,
)
...
# Merged-in ML weather-forecasting layer (XGBoost, see backend/weather_ml/).
# Kept as its own router with its own model family and location registry --
# see app/weather_ml_routes.py for why it's isolated from the routes below.
...
def load_models() -> None:
    ...
    # Separate model family (XGBoost weather forecasting) -- its own
    # try/except so a missing weather_ml model never takes down the
    # crowd-prediction API above; it just leaves /weather/forecast/* at 503.
    load_weather_ml_engine()
...
# ┌───────────────────────────────────────────────────────────────────┐
# │  ROUTES · LIVE WEATHER (real external network call)                 │
# │                                                                       │
# │  The one route that reaches out over the internet in real time, to │
# │  Open-Meteo (see utils/live_weather.py). Never blocks or fails the  │
# │  rest of the app: a dead network simply means available: false.     │
# └───────────────────────────────────────────────────────────────────┘

@cache.cached(ttl_seconds=600, prefix="live_weather")  # short TTL -- this is live data
def _cached_live_weather(place_name: str) -> dict:
    lat, lon = predictor.get_coordinates(place_name)
    data = live_weather.get_live_conditions(lat, lon)
    if data is None:
        return {
            "place_name": place_name, "available": False, "provider": None,
            "current": None, "forecast": [],
            "message": "Live weather is temporarily unavailable -- showing historical "
                       "seasonal averages instead (see /predict for those).",
        }
    return {
        "place_name": place_name, "available": True, "provider": data["provider"],
        "current": data["current"], "forecast": data["forecast"],
        "message": f"Live conditions from {data['provider']}, right now.",
    }


@app.get("/weather/live", response_model=LiveWeatherResponse, tags=["weather"])
def weather_live(place: str = Query(..., description="Place name, e.g. 'Taj Mahal'")):
    """Real-time weather for a place, fetched live from Open-Meteo (free,
    no API key). Complements /predict (historical monthly averages) with
    "what's it actually like there today" for near-term travellers."""
    _require_models()
    try:
        resolved = predictor.resolve_place_name(place)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    return _cached_live_weather(resolved)
...
def health():
    return HealthResponse(
        ...
        version=config.API_VERSION,
        weather_ml_loaded=weather_ml_routes._engine is not None,
    )
